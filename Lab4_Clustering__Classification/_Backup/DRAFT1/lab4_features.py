...
FEATURE = "PIXEL" # {PIXEL, COLORHIST, HOG, DAISY}
ALGORITHM = "KMEANS" # {KMEANS, AFFINITYPROP(Not Tested), MINIBATCHKMEANS, MEANSHIFT (Not Tested)}
...
if ALGORITHM == "AFFINITYPROP":
    ...
    kf = k_means.fit(training_flat)
    centroids = kf.cluster_centers_indices
    print ALGORITHM, ",Clusters:", len(k_means.cluster_centers_)
    ...
    k_means= MeanShift()
    kf = k_means.fit(training_flat)
    centroids = kf.cluster_centers_indices_
    print ALGORITHM, ",Clusters:", len(k_means.cluster_centers_)
else:
...
start_time = timer()
training_accuracy = evaluate_accuracy(centroids, k_means.labels_, training, training_flat, training_lbls, dataset_label_names)
...
start_time = timer()
testing_accuracy = evaluate_accuracy(centroids, testing_prediction_lbls, testing, testing_flat, testing_lbls, dataset_label_names)
